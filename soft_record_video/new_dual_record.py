...
Запись продолжается до ручной остановки (Ctrl+C)
"""
...
class DualCameraRecorder:
    ...
    def run_continuous_recording(self, codec='h264'):
        """Запуск непрерывной записи до ручной остановки"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        self.left_file = f"camera_left_{timestamp}.mp4"
        self.right_file = f"camera_right_{timestamp}.mp4"
        
        print("🎬 НЕПРЕРЫВНАЯ ЗАПИСЬ 4K КАМЕР")
        print("=" * 40)
        print(f"[INFO] 🚀 Запись двух 4K камер до ручной остановки")
        ...
        # Подключаем обработчик остановки
    ...
    def analyze_results(self):
        ...
        if os.path.exists(self.left_file):
            ...
            if size_left < 5 * 1024 * 1024:  # меньше 5 МБ
                ...
                # Примерный bitrate
                actual_bitrate = (size_left * 8) / actual_duration / 1000000
        ...
        if os.path.exists(self.right_file):
            ...
            if size_right < 5 * 1024 * 1024:  # меньше 5 МБ
                ...
                # Примерный bitrate
                actual_bitrate = (size_right * 8) / actual_duration / 1000000
        ...
        if success:
            ...
            print(f"[INFO] 📊 Общий размер: {total_mb} МБ ({total_gb:.2f} ГБ)")
    ...
    def stop(self):
        """Переопределенный метод остановки с вызовом callback"""
        # Вызываем оригинальный метод
        super().stop() if hasattr(super(), 'stop') else None
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Непрерывная двойная запись 4K камер')
    ...
    parser.add_argument('--duration', type=int, default=0,
                       help='Длительность записи в секундах (0 = до ручной остановки)')
    ...
    success = recorder.run_continuous_recording(args.codec)
