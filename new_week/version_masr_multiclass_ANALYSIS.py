...
"""
    ...
    def playback_draw_probe(self, pad, info, u_data):
        ...
        try:
            ...
            # Получаем детекцию мяча для текущего момента (для текстового вывода)
            det = self.history.get_detection_for_timestamp(pts_sec, max_delta=0.12)

            # Получаем ВСЕ детекции всех классов для отрисовки
            all_detections = self.get_all_detections_for_timestamp(pts_sec, max_delta=0.12)

            # ========== РЕЖИМ ПАНОРАМЫ: добавляем интерполированные точки мяча ==========
            # В режиме панорамы показываем ВСЕ точки включая интерполяцию
            # В режиме камеры - только реальные детекции (экономия ресурсов)
            if self.display_mode == 'panorama' and det:
                # Если нашли детекцию мяча из истории, добавляем её в all_detections
                if all_detections is None:
                    all_detections = {'ball': [], 'player': [], 'staff': [], 'referee': []}

                # Формируем детекцию в формате для отрисовки
                ball_det = {
                    'x': det[6] if len(det) > 6 else det[0],  # глобальная X
                    'y': det[7] if len(det) > 7 else det[1],  # глобальная Y
                    'width': det[8] if len(det) > 8 else det[2],
                    'height': det[9] if len(det) > 9 else det[3],
                    'confidence': det[4] if len(det) > 4 else 0.5,
                    'is_interpolated': True  # Маркер для визуального отличия
                }

                # Проверяем, нет ли уже такой же детекции (чтобы не дублировать)
            ...
            # Вычисляем сглаженный центр масс используя всю историю (7 секунд)
            ...
            while l_frame:
                ...
                # Отрисовка тайлов ОТКЛЮЧЕНА (освобождаем место для детекций)
                # TILE_POSITIONS содержит (x, y, width, height) для каждого тайла
                num_tiles = 0  # Отключили тайлы

                # Подсчитываем сколько бокс нам нужно нарисовать
                # ТОЛЬКО МЯЧ И ИГРОКИ (персонал и судьи отключены)
                num_detection_rects = 0
                if all_detections:
                    num_detection_rects = (len(all_detections.get('ball', [])) +
                                          len(all_detections.get('player', [])))

                # Ограничиваем общее количество прямоугольников
                # ВАЖНО: На этой платформе лимит rect_params = 16
                # 0 тайлов + максимум 16 детекций
                ...
                # Тайлы отключены - сразу переходим к детекциям

                # Добавляем только основной текст (без подписей тайлов)
                display_meta.num_labels = 1  # только основной текст

                # Рисуем детекции ТОЛЬКО мяча и игроков
                rect_idx = 0  # Начинаем с 0, т.к. нет тайлов
                total_drawn = 0

                if all_detections:
                    # Цветовая схема: только мяч и игроки
                    # ball: красный (1.0, 0.0, 0.0, 1.0)
                    # player: зелёный (0.0, 1.0, 0.0, 1.0)
                    ...
                    class_widths = {
                        'ball': 3,      # Толще для мяча
                        'player': 2     # Потолще для видимости
                    }

                    # Ограничиваем количество рисуемых боксов
                    max_boxes = num_detection_rects

                    for class_name, color in class_colors.items():
                        detections_list = all_detections.get(class_name, [])

                        # Для игроков рисуем центр масс вместо индивидуальных боксов
                        if class_name == 'player' and self.players_center_mass_smoothed:
                            # Используем предвычисленную сглаженную позицию из _compute_smoothed_center_of_mass()
                            # Эта функция использует всю 7-секундную историю для максимально плавного движения
                            center_x, center_y = self.players_center_mass_smoothed

                            # Размер бокса центра масс (можно настроить)
                            cm_box_size = 100  # 100x100 пикселей
                            ...
                            if rect_idx < max_boxes and rect_idx < display_meta.num_rects:
                                try:
                                    ...
                                    rect.border_width = 4  # Потолще для центра масс
                                    rect.border_color.set(*color)  # Зелёный цвет
                                    rect.has_bg_color = 0
                        ...
                        # Для мяча рисуем как обычно
                            ...
                            for d in detections_list:
                                ...
                                try:
                                    ...
                                    # Интерполированные точки - жёлтые с такой же толщиной
                                    if is_interp:
                                        rect.border_width = class_widths[class_name]  # Такая же толщина
                                        rect.border_color.set(1.0, 1.0, 0.0, 1.0)  # Жёлтый яркий
                                    else:
                                        rect.border_width = class_widths[class_name]
                                        rect.border_color.set(*color)  # Красный для реальных
                                    ...
                                    # Достигнут лимит rect_params, останавливаемся
                                    logger.warning(f"Reached rect_params limit at index {rect_idx}: {e}")
                                    break
                ...
                if all_detections:
                    # Показываем только мяч и игроков
                    text += f" | Drawn: Ball={len(all_detections.get('ball', []))}, Players={len(all_detections.get('player', []))}"

                # Добавляем основной текст (FPS, буфер, мяч)
                lbl = display_meta.text_params[0]  # Индекс 0, т.к. нет тайлов
