...
static gboolean ring_push_copy       (GstNvdsRingBuf *self, NvBufSurface *in_surf, GstClockTime pts, GstClockTime dts, GstClockTime duration);
static gboolean ring_pop_copy        (GstNvdsRingBuf *self, NvBufSurface *out_surf, GstClockTime *pts, GstClockTime *dts, GstClockTime *duration);
...
gst_nvds_ringbuf_transform_ip (GstBaseTransform * trans, GstBuffer * buf)
{
  ...
  GST_INFO_OBJECT (self, ">>> ENTER transform_ip, accumulated=%u, delay_complete=%d, head=%u, tail=%u",
                  self->accumulated, self->delay_complete, self->head, self->tail);
  ...
  if (!surf || surf->batchSize < 1) {
    GST_ERROR_OBJECT (self, "Invalid NvBufSurface (batchSize=%d)", surf ? surf->batchSize : -1);
    gst_buffer_unmap (buf, &map);
    return GST_FLOW_ERROR;
  }

  GST_INFO_OBJECT (self, "Mapped buffer OK, batchSize=%d", surf->batchSize);

  g_rec_mutex_lock (&self->lock);

  // Получаем метаданные из GstBuffer
  GstClockTime in_pts = GST_BUFFER_PTS (buf);
  GstClockTime in_dts = GST_BUFFER_DTS (buf);
  GstClockTime in_duration = GST_BUFFER_DURATION (buf);

  // Копируем входной кадр в кольцо (пиксели + метаданные)
  GST_LOG_OBJECT (self, "Calling ring_push_copy (head=%u)...", self->head);
  if (!ring_push_copy (self, surf, in_pts, in_dts, in_duration)) {
    GST_ERROR_OBJECT (self, "Failed to push to ring");
    g_rec_mutex_unlock (&self->lock);
    gst_buffer_unmap (buf, &map);
    return GST_FLOW_ERROR;
  }
  GST_LOG_OBJECT (self, "ring_push_copy OK, new head=%u", self->head);

  self->accumulated++;
  GST_LOG_OBJECT (self, "accumulated incremented to %u (size=%u)", self->accumulated, self->size);

  // Проверяем, накопили ли достаточно кадров для задержки
  if (!self->delay_complete) {
    GST_LOG_OBJECT (self, "delay_complete=FALSE, checking if accumulated(%u) >= size(%u)",
    ...
    if (self->accumulated >= self->size) {
      ...
      GST_INFO_OBJECT (self, "🎯 Buffer filled! Starting delayed output after %u frames",
                       self->accumulated);
      GST_INFO_OBJECT (self, "   head=%u, tail=%u, will start popping from tail", self->head, self->tail);
    } else {
      // Ещё накапливаем - пропускаем кадр дальше (pass-through)
      g_rec_mutex_unlock (&self->lock);
      gst_buffer_unmap (buf, &map);

      // Каждые 30 кадров сообщаем о прогрессе
      if (self->accumulated % 30 == 0) {
        GST_INFO_OBJECT (self, "📥 Accumulating: %u/%u frames (%.1f/%.1f sec)",
                         self->accumulated, self->size,
                         (float)self->accumulated / 30.0,
                         (float)self->size / 30.0);
      }

      GST_INFO_OBJECT (self, "<<< EXIT transform_ip: PASS-THROUGH (accumulating)");
      return GST_FLOW_OK;  // Pass-through: пропускаем кадр дальше
    }
  }

  // Буфер заполнен - выдаём старый кадр из хвоста
  if (self->delay_complete) {
    GST_LOG_OBJECT (self, "delay_complete=TRUE, calling ring_pop_copy (tail=%u)...", self->tail);

    GstClockTime out_pts, out_dts, out_duration;
    if (!ring_pop_copy (self, surf, &out_pts, &out_dts, &out_duration)) {
    ...
    // Восстанавливаем старые метаданные в GstBuffer
    GST_BUFFER_PTS (buf) = out_pts;
    GST_BUFFER_DTS (buf) = out_dts;
    GST_BUFFER_DURATION (buf) = out_duration;

    GST_LOG_OBJECT (self, "ring_pop_copy OK, new tail=%u, restored PTS=%" GST_TIME_FORMAT,
                    self->tail, GST_TIME_ARGS(out_pts));

    // Логирование каждые 30 кадров
    if (self->accumulated % 30 == 0) {
      GST_INFO_OBJECT (self, "📤 Output frame %u with delay of %u frames (%.1f sec), head=%u, tail=%u, PTS=%" GST_TIME_FORMAT,
                       self->accumulated, self->size, (float)self->size / 30.0,
                       self->head, self->tail, GST_TIME_ARGS(out_pts));
    }
  }
  ...
  GST_INFO_OBJECT (self, "<<< EXIT transform_ip: FLOW_OK");
  return GST_FLOW_OK;
}
...
static gboolean
ring_push_copy (GstNvdsRingBuf *self, NvBufSurface *in_surf, GstClockTime pts, GstClockTime dts, GstClockTime duration)
{
  GST_LOG_OBJECT (self, "  >> ring_push_copy: head=%u, size=%u, pts=%" GST_TIME_FORMAT,
                  self->head, self->size, GST_TIME_ARGS(pts));

  if (!self->slots || self->size == 0) {
    GST_ERROR_OBJECT (self, "  !! ring_push_copy: slots=%p, size=%u (invalid)", self->slots, self->size);
    return FALSE;
  }

  RingSlot *dst = (RingSlot*) g_ptr_array_index (self->slots, self->head);
  if (!dst || !dst->surf) {
    GST_ERROR_OBJECT (self, "  !! ring_push_copy: slot[%u] is NULL or has NULL surf", self->head);
    return FALSE;
  }
  ...
  GST_LOG_OBJECT (self, "  >> Calling NvBufSurfTransform...");
  if (NvBufSurfTransform (&src_wrap, dst->surf, &params) != 0) {
    GST_ERROR_OBJECT (self, "  !! NvBufSurfTransform failed in ring_push_copy");
    return FALSE;
  }

  // Сохраняем метаданные вместе с пикселями
  dst->pts = pts;
  dst->dts = dts;
  dst->duration = duration;

  // Сдвигаем head
  guint old_head = self->head;
  self->head = (self->head + 1) % self->size;
  GST_LOG_OBJECT (self, "  << ring_push_copy: pushed to slot %u, head moved %u -> %u",
                  old_head, old_head, self->head);
  return TRUE;
}

static gboolean
ring_pop_copy (GstNvdsRingBuf *self, NvBufSurface *out_surf, GstClockTime *pts, GstClockTime *dts, GstClockTime *duration)
{
  GST_LOG_OBJECT (self, "  >> ring_pop_copy: tail=%u, size=%u", self->tail, self->size);

  if (!self->slots || self->size == 0) {
    GST_ERROR_OBJECT (self, "  !! ring_pop_copy: slots=%p, size=%u (invalid)", self->slots, self->size);
    return FALSE;
  }

  RingSlot *src = (RingSlot*) g_ptr_array_index (self->slots, self->tail);
  if (!src || !src->surf) {
    GST_ERROR_OBJECT (self, "  !! ring_pop_copy: slot[%u] is NULL or has NULL surf", self->tail);
    return FALSE;
  }
  ...
  GST_LOG_OBJECT (self, "  >> Calling NvBufSurfTransform (pop)...");
  if (NvBufSurfTransform (src->surf, &dst_wrap, &params) != 0) {
    GST_ERROR_OBJECT (self, "  !! NvBufSurfTransform failed in ring_pop_copy");
    return FALSE;
  }

  // Восстанавливаем метаданные из буфера
  if (pts) *pts = src->pts;
  if (dts) *dts = src->dts;
  if (duration) *duration = src->duration;

  // Сдвигаем tail
  guint old_tail = self->tail;
  self->tail = (self->tail + 1) % self->size;
  GST_LOG_OBJECT (self, "  << ring_pop_copy: popped from slot %u (pts=%" GST_TIME_FORMAT "), tail moved %u -> %u",
                  old_tail, GST_TIME_ARGS(src->pts), old_tail, self->tail);
  return TRUE;
}
